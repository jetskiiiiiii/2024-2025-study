...
try:  # check if we're in a git repo
    repo_dir = (
        subprocess.run(["git", "rev-parse", "--show-toplevel"], capture_output=True, check=True).stdout.decode().strip()
    )
...
def _clone_repo(repo, branch, prefix):
    ...
    url = "https://github.com/jetskiiiiiii/fsdl-text-recognizer-2022-labs.git"
    subprocess.run(["git", "clone", "--branch", branch, "-q", url], check=True)  # run git clone
...
def _install_dependencies_colab():
    ...
    subprocess.run(["sed 1d requirements/dev.in | grep -v '#' | xargs pip install --quiet"], shell=True, check=True)
